...
class DuelingDQN(nn.Module):
    ...
    def __init__(self, state_size: int, action_size: int, 
                 feature_hidden_units: List[int] = [128, 128, 64], 
                 value_hidden_units: List[int] = [64, 32],       
                 advantage_hidden_units: List[int] = [64, 32]): 
        ...
        for hidden_size in feature_hidden_units:
            feature_layers_list.extend([
        ...
        self.feature_layers = nn.Sequential(*feature_layers_list)
        
        # Value Stream
        # Estimates V(s)
        value_stream_layers = []
        last_feature_layer_size = feature_hidden_units[-1]
        input_value_size = last_feature_layer_size
        for hidden_value_size in value_hidden_units:
            value_stream_layers.extend([
                nn.Linear(input_value_size, hidden_value_size),
                nn.LeakyReLU()
            ])
            input_value_size = hidden_value_size
        value_stream_layers.append(nn.Linear(input_value_size, 1)) # Output layer for V(s)
        
        self.value_stream = nn.Sequential(*value_stream_layers)
        
        # Advantage Stream 
        # Estimates A(s,a)
        advantage_stream_layers = []
        input_advantage_size = last_feature_layer_size
        for hidden_advantage_size in advantage_hidden_units:
            advantage_stream_layers.extend([
                nn.Linear(input_advantage_size, hidden_advantage_size),
                nn.LeakyReLU()
            ])
            input_advantage_size = hidden_advantage_size
        advantage_stream_layers.append(nn.Linear(input_advantage_size, action_size)) 

        self.advantage_stream = nn.Sequential(*advantage_stream_layers)
...
class DuelingDQNAgent:
    ...
    def __init__(self, state_size: int, action_size: int, config=None):
        ...
        self.q_network = DuelingDQN(
            state_size, 
            action_size, 
            feature_hidden_units=config.FEATURE_HIDDEN_UNITS,
            value_hidden_units=config.STREAM_HIDDEN_UNITS,
            advantage_hidden_units=config.STREAM_HIDDEN_UNITS 
        ).to(self.device)
...
def prepare_data(df: pd.DataFrame, max_rows: int = 1000) -> pd.DataFrame:
...
def main():
    ...
    # Train
    trainer.train(num_episodes=100, save_interval=1, validate_interval=1)
